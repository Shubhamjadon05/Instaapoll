...
@Component({
  selector: 'app-polling-rating',
  templateUrl: './polling-rating.component.html',
  styleUrls: ['./polling-rating.component.css']
})

export class PollingRatingComponent implements OnInit{
  users:any
  rating:any 
  formData = {
  ...
  rater_count(){
    this.ratingservice.getPollResult({event_id:this.formData.Event}).subscribe((response:any)=>{
this.RaterCount=response.count
  ...
  Rating_data(){
    ...
    this.ratingservice.getOptions({event_id:this.formData.Event}).subscribe((data: any)=>{
