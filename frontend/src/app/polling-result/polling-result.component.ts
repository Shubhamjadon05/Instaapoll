...
@Component({
  selector: 'app-polling-result',
  templateUrl: './polling-result.component.html',
  styleUrls: ['./polling-result.component.css']
})

export class PollingResultComponent implements OnInit{
  users:any
  rating:any 
  formData = {
  ...
  rater_count(){
    this.ratingservice.getPollResultForOption({event_id:this.formData.Event}).subscribe((response:any)=>{
this.RaterCount=response.count
  ...
  Rating_data(){
    ...
    this.ratingservice.getVoters({event_id:this.formData.Event}).subscribe((data: any)=>{
      console.log(data, 'pollingdata')
